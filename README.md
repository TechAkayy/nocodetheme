...
A custom Vite plugin, `vite-plugin-pg.js`, serves this purpose and utilizes environment variables (`LIB` and `WP`) passed from the package.json script commands (`watch` and `watch-wp`). The `WP` environment variable is then employed in `tailwind.config.js` to conditionally enable or disable Tailwind preflight.
